    ...
    return (
        <div className="bg-white dark:bg-dark-surface rounded-2xl border border-gray-200 dark:border-gray-800 p-6" onClick={() => fileInputRef.current?.click()}>
            ...
            {image ? (
                ...
                <div className="border-2 border-dashed border-gray-200 dark:border-gray-700 rounded-xl p-10 text-center hover:border-gray-400 dark:hover:border-gray-500 hover:bg-gray-50/50 dark:hover:bg-gray-800/10 transition-all cursor-pointer group">
                    <div className="w-14 h-14 rounded-2xl bg-gray-100 dark:bg-gray-800 group-hover:bg-gray-200 dark:group-hover:bg-gray-700 flex items-center justify-center mx-auto mb-3 transition-colors">
                        <FontAwesomeIcon icon={faImage} className="text-2xl text-gray-400 dark:text-gray-500 group-hover:text-gray-800 dark:group-hover:text-gray-300 transition-colors" />
                    </div>
                    <p className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">Kéo thả ảnh vào đây</p>
                    <p className="text-xs text-gray-400 dark:text-gray-500">
                        hoặc <span className="text-gray-900 dark:text-white hover:underline font-medium">click để chọn</span> — PNG, JPG, WebP (tối đa 5MB)
                    </p>
                </div>
            )}
            <input
